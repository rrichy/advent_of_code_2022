...
fn get_monkeys(input: String) -> Vec<Monkey> {
    ...
    for details in input.split("\r\n\r\n") {
        ...
        let items: Vec<u64> = caps.get(1).unwrap().as_str().split(", ").map(|n| n.parse().unwrap()).collect();
        ...
        monkeys.push(Monkey { items, operation, number, modulus, if_true, if_false, inspect: 0 });
    }
    ...
    fn play(&mut self, modulus: &u32) -> Vec<(usize, u64)> {
        ...
        for item in &self.items {
            ...
            match worry % (self.modulus as u64) == 0 {
                true => { pass.push((self.if_true.clone(), worry)); },
                false => { pass.push((self.if_false.clone(), worry)); },
            }
        }
